...
def main():
    ...
    ####################################################
    # Initialize TrajCrafterAutoregressive
    ####################################################
    ...
    #################################################
    # Take models from TrajCrafter
    #################################################
    
    text_encoder = trajcrafter.text_encoder
    vae = trajcrafter.vae
    transformer3d = trajcrafter.transformer3d
    ...
    print(f"Original model has {len(transformer.transformer_blocks)} layers")

    # Keep only first 16 layers
    transformer.transformer_blocks = transformer.transformer_blocks[:num_layers_to_keep]

    # # Freeze first 8 layers
    # for i in range(num_frozen_layers):
    #     for param in transformer.transformer_blocks[i].parameters():
    #         param.requires_grad = False
    #     print(f"Frozen layer {i}")

    # # Ensure layers 8-15 are trainable
    # for i in range(num_frozen_layers, num_layers_to_keep):
    #     for param in transformer.transformer_blocks[i].parameters():
    #         param.requires_grad = True
    #     print(f"Trainable layer {i}")

    # Update cross-attention if needed
    if hasattr(transformer, 'is_train_cross') and transformer.is_train_cross:
        if hasattr(transformer, 'perceiver_cross_attention'):
            transformer.perceiver_cross_attention = transformer.perceiver_cross_attention[:num_cross_layers_to_keep]

            print(f"Updated cross-attention layers to {num_cross_layers_to_keep}")
            
            # transformer.perceiver_cross_attention[0].requires_grad_(False)
            # print(f"Frozen cross-attention layer 0")
            
            # transformer.perceiver_cross_attention[1].requires_grad_(True)
            # print(f"Trainable cross-attention layer 1")

    print(f"Model now has {len(transformer.transformer_blocks)} layers and {len(transformer.perceiver_cross_attention)} cross-attention layers")

    # TODO: can only load the VAE, T5, Crosstransformer from given paths
    # TODO: can create a reduced version of CrossTransformer with 16 layers, with a separate ckpt
    ...
    for name, param in tqdm(network.named_parameters(), desc="Freezing LoRA parameters"):
        if param.requires_grad:
            ...
            if should_freeze:
                param.requires_grad = False
                print(f"Disabled LoRA parameter: {name}")
